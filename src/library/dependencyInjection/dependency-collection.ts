...
export class DependencyCollection {
    ...
    public static get globalCollection(): DependencyCollection {
        if (!this._globalCollection) this._globalCollection = new DependencyCollection();
    ...
    register<T = any>(objectType: ObjectType<T>, lifeTime: DependencyLifeTime, dependencies?: ObjectType[], instance?: T): void {
        if (this.contains(objectType)) throw new Error(`The type ${getObjectTypeName(objectType)} is already registered.`);

        if (instance && lifeTime !== DependencyLifeTime.Singleton) throw new Error(`Only the singletons can be registered with an existing instance.`);
    ...
    get(objectType: ObjectType): DependencyDescriptor {
        if (!this._registeredTypes.has(objectType)) throw new Error(`The type ${getObjectTypeName(objectType)} is not registered.`);
    ...
    private validateDependencyRegistration<T>(objectType: ObjectType<T>): void {
        ...
        for (const dependencyType of descriptor.dependencies) {
            if (!this.contains(dependencyType)) {
                throw new Error(
                    `The type '${getObjectTypeName(objectType)}' depends on the type '${getObjectTypeName(dependencyType)}' but is not registered.`
                );
            }
    ...
    private validateCircularDependency<T>(objectType: ObjectType<T>, hierarchy?: ObjectType[]): void {
        ...
        for (const dependencyType of dependencies) {
            if (!this.contains(dependencyType)) continue;

            if (hierarchy.find(x => x === dependencyType)) {
                throw new Error(
                    `Circular dependency found in service type '${getObjectTypeName(hierarchy[0])}': ${hierarchy
                        .map(x => getObjectTypeName(x))
                        .join(" -> ")} -> ${getObjectTypeName(dependencyType)}.`
                );
            }
    ...
    private validateScopedOnSingletons<T>(objectType: ObjectType<T>): void {
        ...
        for (const dependencyType of descriptor.dependencies) {
            ...
            if (descriptor.lifeTime === DependencyLifeTime.Singleton && dependencyDescriptor.lifeTime === DependencyLifeTime.Scoped) {
